...
class MusicLlama:
    ...
    def build_multi_task(
        ckpt_dir: str,
        lora_dir: str, # Path to the epoch_X folder containing the 3 adapters
        active_adapter: str, # "commu_lora", "emopia_lora", or "slakh_lora"
        model_config_path: str,
        max_seq_len: int,
        max_batch_size: int,
        seed: int = 1,
        ...
        # 2. Load the 3 Multi-Task LoRA Adapters
        ...
        # 3. Activate the chosen adapter
        model.set_adapter(active_adapter)
        print(f"--> Multi-Task PEFT loaded. Active adapter: {active_adapter}")

        if is_xpu_available(): model.to("xpu")
        else: model.to("cuda")
        model.eval()
